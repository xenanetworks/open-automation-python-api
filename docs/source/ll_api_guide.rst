...
However, if the developer needs to migrate a :term:`CLI` script to :term:`XOA`, the LL-API can explicitly show the command name, which may speed up the migration process.
